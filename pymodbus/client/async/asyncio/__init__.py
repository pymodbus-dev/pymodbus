...
class ReconnectingAsyncioModbusUdpClient(object):
    ...
    def start(self, host, port=502):
        ...
        addrinfo = yield from self.loop.getaddrinfo(host, port, type=socket.SocketKind.SOCK_DGRAM)
    ...
    def _connect(self):
        _logger.debug('Connecting.')
        try:
            yield from self.loop.create_datagram_endpoint(functools.partial(self._create_protocol, host=self.host, port=self.port), remote_addr=(self.host, self.port))
            _logger.info('Connected to %s:%s.' % (self.host, self.port))
        except Exception as ex:
            _logger.warning('Failed to connect: %s' % ex)
            asyncio.async(self._reconnect(), loop=self.loop)
...
class AsyncioModbusUdpClient(object):
    ...
    def connect(self):
        _logger.debug('Connecting.')
        try:
            addrinfo = yield from self.loop.getaddrinfo(self.host, self.port, type=socket.SocketKind.SOCK_DGRAM)
            _host, _port = addrinfo[0][-1]
            yield from self.loop.create_datagram_endpoint(functools.partial(self._create_protocol, host=_host, port=_port), remote_addr=(self.host, self.port))
            _logger.info('Connected to %s:%s.' % (self.host, self.port))
        except Exception as ex:
            _logger.warning('Failed to connect: %s' % ex)
            # asyncio.async(self._reconnect(), loop=self.loop)
