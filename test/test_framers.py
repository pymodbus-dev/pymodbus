...
def test_framer_initialization(framer):
    ...
    if isinstance(framer, ModbusAsciiFramer):
        assert framer._header == {'lrc': '0000', 'len': 0, 'uid': 0x00} # pylint: disable=protected-access
        ...
        assert framer._header == {'uid': 0x00, 'len': 0, 'crc': b'\x00\x00'} # pylint: disable=protected-access
        ...
        assert framer._header == {'crc': 0x0000, 'len': 0, 'uid': 0x00} # pylint: disable=protected-access
        assert framer._hsize == 0x01 # pylint: disable=protected-access
        assert framer._start == b'\x7b' # pylint: disable=protected-access
        assert framer._end == b'\x7d' # pylint: disable=protected-access
        assert framer._repeat == [b'}'[0], b'{'[0]] # pylint: disable=protected-access
...
def test_rtu_advance_framer(rtu_framer, data): # pylint: disable=redefined-outer-name
    ...
    assert rtu_framer._header == {'uid': 0x00, 'len': 0, 'crc': b'\x00\x00'} # pylint: disable=protected-access
...
def test_get_raw_frame(rtu_framer): # pylint: disable=redefined-outer-name
    """ Test get raw frame. """
    rtu_framer._buffer = b'\x00\x01\x00\x01\x00\n\xec\x1c' # pylint: disable=protected-access
    assert rtu_framer.getRawFrame() == rtu_framer._buffer # pylint: disable=protected-access
