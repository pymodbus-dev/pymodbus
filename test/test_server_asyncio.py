...
class AsyncioServerTest(asynctest.TestCase):
    ...
    async def testStartTcpServer(self):
        ...
        server = await StartTcpServer(
            context=self.context, loop=self.loop, identity=identity
        )
    ...
    async def testTcpServerReceiveData(self):
        ...
        with patch(
            ...
            await asyncio.sleep(
                0.1
            )  # this may be better done by making an internal hook in the actual implementation
    ...
    async def testTcpServerRoundtrip(self):
        ...
        data = (
            b"\x01\x00\x00\x00\x00\x06\x01\x03\x00\x00\x00\x01"  # unit 1, read register
        )
    ...
    async def testTcpServerNoSlave(self):
        ...
        server = await StartTcpServer(
            context=context, address=("127.0.0.1", 0), loop=self.loop
        )
        server_task = asyncio.create_task(server.serve_forever())
        await server.serving
        connect, receive, eof = (
    ...
    async def testUdpServerException(self):
        ...
        with patch(
            ...
            received_data = None
            random_port = server.protocol._sock.getsockname()[
                1
            ]  # get the random server port
