...
class ModbusTransactionTest(unittest.TestCase):
    ...
    def testCalculateExceptionLength(self):
        for framer, exception_length in [
            ...
            self._tm._set_adu_size()
            self.assertEqual(self._tm._calculate_exception_length(), exception_length)
    ...
    def testExecute(self, mock_time):
        ...
        request.unit_id = 0
        response = tm.execute(request)
        self.assertEqual(response, b"Broadcast write sent - " b"no response expected")
    ...
    def testTLSProcess(self):
        ...
        self.assertRaises(ModbusIOException, lambda: self._tls._process(mock_callback))
